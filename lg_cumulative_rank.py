...
# Load the three CSV files containing the ranks
pr_sg_df = pd.read_csv('export_lg.csv')        # Social Graph Processing Page Rank
pr_te_df = pd.read_csv('url_scores_lg.csv')     # Text Edge Processing Page Rank
pr_sa_df = pd.read_csv('prsa_rank_lg.csv')  # Sentiment Analysis Page Rank
...
# Rename other DataFrames
pr_te_df.rename(columns={'Total Score': 'PRTE'}, inplace=True)  # Renaming Total Score to PRTE
pr_sa_df.rename(columns={'Rank': 'PRSA'}, inplace=True)

# Merge the dataframes on 'URL' using an outer join
...
# Check and fill NaN values with 0 for ranks
...
if all(col in merged_df.columns for col in output_columns):
    ...
    final_df.to_csv('cumulative_page_rank_lg.csv', index=False)

    print("Cumulative page ranks calculated and saved to cumulative_page_rank_lg.csv.")
else:
