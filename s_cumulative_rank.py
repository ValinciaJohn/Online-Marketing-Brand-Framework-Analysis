...
pr_sg_df = pd.read_csv('export_s.csv')        # Social Graph Processing Page Rank
pr_te_df = pd.read_csv('url_scores_s.csv')     # Text Edge Processing Page Rank
pr_sa_df = pd.read_csv('prsa_rank_s.csv')  # Sentiment Analysis Page Rank
...
if all(col in merged_df.columns for col in output_columns):
    ...
    final_df.to_csv('cumulative_page_rank_s.csv', index=False)

    print("Cumulative page ranks calculated and saved to cumulative_page_rank_s.csv.")
else:
